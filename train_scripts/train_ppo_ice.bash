...
if [ -z "$4" ]; then
    ...
    mlagents-learn custom_configs/$config_name.yaml --run-id $exp_foder --env=$build_folder --base-port $PORT --num-envs=$num_envs --no-graphics
     
else
    exp_foder=$4
    echo "resume $exp_foder"
    mlagents-learn custom_configs/$config_name.yaml --run-id $exp_foder --env=$build_folder --resume --base-port $PORT --num-envs=$num_envs --no-graphics
fi
